...
import { addToCartSlice, clearCartSlice, decreaseToCartSlice, getCartSlice, removeCartSlice } from "../../redux/slice/cartSlice";
import './Cart.css'
...
export const Cart = () => {
  ...
  const getCartData = useSelector(
    (state) => state.cartReducer.getCartData
  );
  ...
  const filteredCartData = getCartData?.filter((cartItem) => cartItem?.quantity > 0);
  return (
            ...
            filteredCartData?.map((cartItem) => (
              <div className="cart-item" key={cartItem.id}>
                ...
                <div className="cart-product">
                  <img src={`http://localhost:5000/${cartItem?.product?.image}`} alt={cartItem?.product.name} />
                  <div>
                    <h3>{cartItem.name}</h3>
                    <p>{cartItem.desc}</p>
